...
### 보류(나중에 다시 논의)
- **월 중 구매 비례 과금** — IAP는 SKU=고정가라 동적 비례 불가. 가격대별 SKU 다중 vs 정가 vs 당월 정가 중 택 = **S4.2 가격 전략**. Slot 모델은 칸 수 가변이라 스키마 영향 없음.
...
| **S4.2** | 1차 SKU 확정 + ASC 상품 등록(콘솔) + `.storekit` 로컬 설정 + 가격 전략 결정 |
